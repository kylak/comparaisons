	...
	for (ligne = 0 ; ligne != lignes.length ; ligne++)
	{
		...
		else if (ver_line = lignes[ligne].match(/([0-9]+)/))
		{
			...
			{	
				if (verset != 1)
				{
					console.log(livre+' '+chapitre+' '+verset+' '+old_verset);
				}
			}
...
livre_a[0]='BIBLE BAYARD - NOUVELLE TRADUCTION - '+tlivre+' livres';



//FILE_JSON_TISCH
fs2.writeFileSync('bayard.js', 'bayard='+JSON.stringify(livre_a, null, 1), 'utf8');
