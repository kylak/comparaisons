...
for (livre = 1 ; livre != 28 ; livre++)
{
	...
	//CHAPITRE
	for (chapitre = 1 ; chapitre != sebastien[livre].length ; chapitre++)
	{


tnb++;
total = (sebastien[livre][chapitre].length-1)+total;

//console.log(tnb+' : '+(sebastien[livre][chapitre].length-1)+' : '+ total+' M '+total/tnb);

		//VERSET
		for (verset = 1 ; verset != sebastien[livre][chapitre].length ; verset++)
		{

			phrase			= [];
			phrase_morph	= [];

			verset_sebastien		= sebastien[livre][chapitre][verset];
...
if (livre == 40 && chapitre == 2 && verset == 2)
	{
	console.log(phrase[i]+' '+phrase[i+1])
	console.log(phrase[i+1].match(/\S+/g).length)
	*/
// !phrase[i].match(/[\u00C0-\u00DC]|[\u0041-\u005A]/) && !phrase[i+1].match(/[\u00C0-\u00DC]|[\u0041-\u005A]/)
